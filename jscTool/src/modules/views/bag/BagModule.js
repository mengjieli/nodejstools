...
var BagModule = ModuleBase.extend({
    ...
    getBagData: function () {
        this.itemData = [];
        var data = mainData.bagDataList;//ModuleMgr.inst().getData("ItemModule").items;
        for (var i = 0; i < data.length; i++) {
            //cc.error(i);
            var item = data.getItemAt(i);
            var itemConfig = ModuleMgr.inst().getData("ConfigTableUpdator").getConfigTableValueByKey("item",item.itemid);
            if(itemConfig){
