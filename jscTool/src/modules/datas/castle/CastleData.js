...
var CastleData = DataBase.extend({
    ...
    init:function()
    {
        ...
        cc.log(ProfileData.getInstance()+"城堡id   CastleData初始化城堡data  init");//LoadingModule.CASTLE_ID
    ...
    //        if(this.getNetResource(data0)!=undefined)
    //            EventMgr.inst().dispatchEvent( CastleEvent.UPDATE_RESOURCE,{castleId:data0,resourceId:data1,differ:differ,current:data2});//城堡id 资源id 增减差值 当前值
    //    }
    //},
    ...
    //            if(castleId==block.castleId&&String(blockId)==String(block.index)){
    //                var arrData=[block.castleId,block.index,block.buildId,block.buildLevel,block.state,block.upgradeTime,block.stateParam,block.beginTime,block.remainTime];
    //                dic=new CastleBlockBeanNet(arrData);
    //                break;
    //            }
    //        }
    //    }
    ...
    //        for(var i = 0;i<list.length;i++){
    //            var block = list.getItemAt(i);
    //            if(castleId==block.castleId){
    //                var arrData=new CastleBlockBeanNet([block.castleId,block.index,block.buildId,block.buildLevel,block.state,block.upgradeTime,block.stateParam,block.beginTime,block.remainTime]);
    //                arr.push(arrData);
    //            }
    //        }
    //        for(var i in arr){
    //            var temp = arr[i];
    //            dic[temp._index]=temp;
    //        }
    //    }
    //
    //    return dic;
