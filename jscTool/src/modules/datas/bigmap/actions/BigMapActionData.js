...
BigMapActionData.prototype.recvMapInfo = function (cmd, data) {
    ...
    if (type == 1205001) { //城堡
        ...
        if (!res) {
            ...
            res.receiveInfo(id, accountId, castleId, name, x, y);
            this.addBuild(res);
        } else {
            res.receiveInfo(id, accountId, castleId, name, x, y);
        ...
        for (var i = 0; i < this.rolers.length; i++) {
            if (this.rolers[i].id == id) {
                res = this.rolers[i];
                break;
            }
        }
        if (!res) {
            res = new MapRolerData(type);
            res.receiveInfo(id, accountId, castleId, name, x, y);
            this.addRoler(res);
        } else {
            res.receiveInfo(id, accountId, castleId, name, x, y);
            this.call(BigMapActionData.UPDATE_ROLER, res);
            //this.call(BigMapActionData.UPDATE_ROLER, res);
        }
    }
    return res;
 ...
 * @returns {MapRoler}
 */
BigMapActionData.prototype.addRoler = function (roler) {
    this.rolers.push(roler);
    this.call(BigMapActionData.UPDATE_ROLER, roler);
    return roler;
}
...
BigMapActionData.UPDATE_ROLER = "update_roler";
