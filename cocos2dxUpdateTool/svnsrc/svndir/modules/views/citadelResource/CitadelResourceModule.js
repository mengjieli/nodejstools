...
var CitadelResourceModule = ModuleBase.extend({
    ...
    updateUI:function()
    {
        ...
        for( var i=0; i<len; i++  )
        {
            ...
            posNum.setString( "x: " + Math.floor(data.x) + " Y: " + Math.floor(data.y) );
            ...
            var typeID = ServerMapConfig.getInstance().getBlock(data.x,data.y).type;
            cc.log( typeID );
            ///var pn = this.getSingleCeiling
            ...
            button.setEnabled( !data.isDouble );
            button.setBright( !data.isDouble );
    ...
    {
        var id = landId + "" + resId;
        var landConfig = this._landTable[ id ];
        var v = Number(landConfig.output_perminute);
    ...
    getSingleCeiling:function( landId, resId, tiredId )
    {
        var id = landId + "" + resId;
        var landConfig = this._landTable[ id ];
        var v = Number( landConfig.resource_max );
        var tiredConfig = ModuleMgr.inst().getData("ConfigTableUpdator").getConfigTableValueByKey("Territory_levelup", tiredId );
