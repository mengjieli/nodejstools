...
var HeadModule = ModuleBase.extend({
    ...
    ctor:function() {
        this._super();
        //修改头像网络监听
        NetMgr.inst().addEventListener(298, this.netUpdateAccountInfo, this);//更新账号属性
        //修改头像成功事件监听
    ...
    initUI:function() {
        ...
        left1.setString(SelfData.getInstance().nick);
        ...
        right1.setString(SelfData.getInstance().accountId);
    ...
    destroy:function() {
        NetMgr.inst().removeEventListener(298, this.netUpdateAccountInfo, this);
    ...
    netUpdateAccountInfo: function (cmd, data) {
        if (298 == cmd) {
            data.resetCMDData();
            ...
            if(nick!="") {
                ...
                EventMgr.inst().dispatchEvent(HEADEVENT.CHANGE_NAME_SUCCESS,nick);
            }

            cc.log("HeadModule",accountUuid,nick);

            var len=data.readUint();
            for(var i=0;i<len;i++){
                var slot=data.readUint();
                var value=data.readString();
                cc.log("298属性 info",slot,value);
                if (slot == 2) {
                    SelfData.getInstance().headId = value;
                    EventMgr.inst().dispatchEvent(HEADEVENT.CHANGE_HEAD_SUCCESS,value);
                }
            }
        }
    },

    setHeadIconView: function (file) {
        if (this.headIcon) {
            cc.log("head", file);
            this.headIcon.loadTexture(ResMgr.inst().getIcoPath(file));
        }
    },

    updateHeadIcon: function (event,data) {
        this.setHeadIconView(data);
    },

    updateName: function (event, data) {
        var left1 = this._panel_part1.getChildByName("Text_3");
        left1.ignoreContentAdaptWithSize(true);
        left1.setString(data);
    },
