 ...
 */
var PrivateChatTfNode = ccui.Widget.extend({

    _imgHead:null,//头像
    _imgHeadBox:null,//头像框
    //_tfName:null,//名字
    _imgWordsBg:null,//说话文本背景
    _tfTime:null,//时间
    _tfWords:null,//说话文本
    ...
    initData:function(node,data){
        ...
        this._imgWordsBg=this._node.getChildByName("words_bg");
        ...
        this.showString(netChat._message);
    ...
    setWordsHeight:function(str){
        ...
        if(this._lineNum>1){
            var offy=(this._lineNum-1)*this._cnH;
            //cc.log("xy",this._tfWords.y);
            this._tfWords.setContentSize(cc.size(this._tfWords.width,this._tfWords.height+offy));
            this._imgWordsBg.setContentSize(cc.size(this._imgWordsBg.width,this._tfWords.height+10));

            if(this._lineNum>3){
                var offy=(this._lineNum-3)*this._cnH;
                this._node.setContentSize(cc.size(this._node.width,this._tfWords.getContentSize().height+30));
