module flower {
    ...
    export class TweenPath implements IPlugin {
        ...
        public static to(target:any, time:number, path, ease?:string):Tween {
            return Tween.to(target, time, {"path": path}, ease);
        }

        public static vto(target:any, v:number, path, ease?:string):Tween {
            var sum = 0;
            for(var i = 1, len = path.length; i < len; i++) {
